WITH all_txns as (SELECT account, "tradeAmount"/10^6 as vol, timestamp FROM "Transactions"
     ...
     may_cohort_b AS (SELECT * FROM may_accounts WHERE vol > 100),
     may_cohort_c AS (SELECT * FROM may_accounts WHERE vol > 1000),
     may_cohort_d AS (SELECT * FROM may_accounts WHERE vol > 5000),
     may_cohort_e AS (SELECT * FROM may_accounts WHERE vol > 10000),
     may_cohort_f AS (SELECT * FROM may_accounts WHERE vol > 50000),
     may_cohort_g AS (SELECT * FROM may_accounts WHERE vol > 100000),
     may_cohort_h AS (SELECT * FROM may_accounts WHERE vol > 250000),
     may_cohort_i AS (SELECT * FROM may_accounts WHERE vol > 500000),
     may_cohort_j AS (SELECT * FROM may_accounts WHERE vol > 1000000),
     may_cohort_k AS (SELECT * FROM may_accounts WHERE vol > 5000000),
